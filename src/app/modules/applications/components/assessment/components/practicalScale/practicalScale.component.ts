...
import { PRACTICALSKILLS } from '../../../../../../model/practicalSkill';
...
@Component({
  selector: 'app-practicalScale',
  templateUrl: './practicalScale.component.html',
  styleUrls: ['./practicalScale.component.scss']
})
export class PracticalScaleComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.practicalform = this.formBuilder.group({
  ...
  onChange(skill: string, isChecked: boolean) {
    const skillsFormArray = <FormArray>this.practicalform.controls.skills;
    if (isChecked) {
  ...
  triggerView(event:any){
    ...
    this.dataService.changeAssessmentView('RESEARCHVIEW');
  }
