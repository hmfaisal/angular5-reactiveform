...
import { FASHIONSKILLS } from '../../../../../../model/fashionSkill';
...
@Component({
  selector: 'app-fashionScale',
  templateUrl: './fashionScale.component.html',
  styleUrls: ['./fashionScale.component.scss']
})
export class FashionScaleComponent implements OnInit, OnDestroy {

  fashionform: FormGroup;
  skills=FASHIONSKILLS ;
  scaleValue = 0;
  submitted = false;
  ...
  onChange(skill: string, isChecked: boolean) {
    const skillsFormArray = <FormArray>this.fashionform.controls.skills;
    if (isChecked) {
  ...
  triggerView(event:any){
    ...
    this.dataService.changeAssessmentView('LABORVIEW');
  }
